...
def settings():
    return SimpleNamespace(
        ...
        max_concurrent_sentiment=5,
        max_concurrent_intent=5,
        max_concurrent_ingest_per_agent=3,
        max_concurrent_insights_per_agent=2,
        max_concurrent_sentiment_per_agent=2,
        max_concurrent_intent_per_agent=2,
    )
...
def test_cannot_start_ingest_when_insights_fill_global(monkeypatch, settings):
    monkeypatch.setattr(job_concurrency, "get_settings", lambda: settings)
    db = MagicMock()
    db.scalar.side_effect = [4, 6, 0, 0]  # total 10 == global cap
    assert job_concurrency.can_start_job(db, "ingest") is False
...
def test_can_start_insights_until_limit(monkeypatch, settings):
    monkeypatch.setattr(job_concurrency, "get_settings", lambda: settings)
    db = MagicMock()
    db.scalar.side_effect = [2, 4, 0, 0]  # insights 4 < 5, total 6 < 10
...
def test_per_agent_ingest_limit_blocks(monkeypatch, settings):
    monkeypatch.setattr(job_concurrency, "get_settings", lambda: settings)
    db = MagicMock()
    db.scalar.side_effect = [4, 0, 0, 0, 3]
...
def test_per_agent_ingest_under_limit_allows(monkeypatch, settings):
    monkeypatch.setattr(job_concurrency, "get_settings", lambda: settings)
    db = MagicMock()
    db.scalar.side_effect = [4, 0, 0, 0, 2]
...
def test_per_agent_insights_limit_blocks(monkeypatch, settings):
    monkeypatch.setattr(job_concurrency, "get_settings", lambda: settings)
    db = MagicMock()
    db.scalar.side_effect = [0, 3, 0, 0, 2]
...
def test_fail_orphaned_jobs_marks_running_failed():
    ...
    sentiment_job = SimpleNamespace(agent_id="a", status="running", error=None, completed_at=None)
    intent_job = SimpleNamespace(agent_id="a", status="running", error=None, completed_at=None)
    db.scalars.return_value.all.side_effect = [[ingest_job], [insights_job], [sentiment_job], [intent_job]]
